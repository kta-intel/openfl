...
class DiGress(PyTorchTaskRunner):
    def __init__(self, device="cpu", precision=32, pretrained_regressor_path=None, **kwargs):
        """
        Initialize the model with the specified device, precision, and optional pretrained regressor.

        Args:
            device (str): The compute device to use for training. Defaults to "cpu".
            precision (int): The precision to use for computation. Defaults to 32.
            pretrained_regressor_path (str, optional): Path to a pretrained regressor model.
    ...
    def validate_task(
        ...
        if getattr(self, 'regressor', None):
            if getattr(self, 'pretrained_regressor_path', None) is None:
                ...
                output_tensor_dict = self._add_save_metric(output_tensor_dict, origin, round_num, tags, validity_metric, kwargs)
    ...
    def _add_save_metric(self, output_tensor_dict, origin, round_num, tags, validity_metric, kwargs):
        """
        Add a validity metric to the output tensor dictionary with appropriate tags.

        This method updates the output tensor dictionary with a validity metric. If the 'apply' key in
        kwargs is set to 'local', the metric is tagged for local validation. Otherwise, it is tagged as
        'validate_agg', indicating that it will be used to check the best model during aggregation.

        Args:
            output_tensor_dict (dict): The dictionary of output tensors to be updated with the validity metric.
            origin (str): The entity that the tensor key originates from (i.e. which collaborator).
            round_num (int): The current round number.
            tags (tuple): The initial set of tags for the tensor key.
            validity_metric (Metric): The validity metric to be added to the output tensor dictionary.
            kwargs (dict): Additional keyword arguments that may control the behavior of metric tagging.

        Returns:
            dict: The updated output tensor dictionary containing the new validity metric.
        """
        if kwargs["apply"] == "local":
            output_tensor_dict[

...
class OpenFLClient:
    ...
    def send_message_to_server(self, flower_message):
        ...
        # print(f"Converting Flower message to OpenFL message and sending to OpenFL server: {openfl_message.message_type}")
        # Send the OpenFL message to the OpenFL server and get a response
        openfl_response = self.stub.Exchange(openfl_message)
        # print(f"Received OpenFL response from OpenFL server: {openfl_response.message_type}")
